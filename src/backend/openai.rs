...
async fn call(
    ...
    let (message, resolved_model) = tokio::time::timeout(
        Duration::from_secs(entry.timeout_secs),
        collect_stream_text(&entry.model, messages, options),
    )
    .await
    .map_err(|_elapsed| {
...
///
/// Also captures the `model` field of the first chunk carrying real content, returned
/// alongside the text as the resolved model: a gateway like OmniRoute can route
/// `entry.model` (an alias `ccm` requested) to a different real upstream model, which
/// a chunk's own `model` field reports, and the caller folds this into the
/// `Commit message generated by…`/`Generated empty message…` progress line (see
/// `generation.rs`) rather than this function logging it directly. Deliberately NOT
/// the first chunk overall: a slow upstream can make OmniRoute send periodic
/// keep-alive chunks while it waits (observed on the wire:
/// `{"id":"omniroute-keepalive","model":"omniroute","choices":
...
async fn collect_stream_text(
    ...
    while let Some(chunk) = stream.next().await {
        ...
        if resolved_model.is_none() && !chunk.model.is_empty() && has_real_delta {
